...
import type {TypeaheadSelectOption} from "../src/select/util/selectTypes";
...
export const TypeaheadStory: StoryObj = {
  name: "Typeahead",
  render: () => {
    const initialState = {
      options: [
        ...
        <div style={{maxWidth: "500px"}}>
          ...
          <StateProvider initialState={initialState}>
            {(state, setState) => (
              <FormField
                label={
                  "Select Languages (API Fetch Simulation) - with keyword by parent"
                }>
                <TypeaheadSelect
                  shouldFilterOptionsByKeyword={false}
                  areOptionsFetching={state.areOptionsFetching}
